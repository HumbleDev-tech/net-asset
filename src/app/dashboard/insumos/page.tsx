...
import { TipoInsumo } from "@/generated/prisma/enums";

type InsumoItem = (typeof mockInsumos)[number] & { stockActual: number };
...
function stockStatus(actual: number, minimo: number) {
  if (actual === 0) return { label: "Sin stock", class: "bg-red-500/15 text-red-400 border-red-500/30", barColor: "bg-red-500" };
  if (actual <= minimo) return { label: "Bajo", class: "bg-amber-500/15 text-amber-400 border-amber-500/30", barColor: "bg-amber-500" };
  return { label: "OK", class: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30", barColor: "bg-emerald-500" };
}
...
export default function InsumosPage() {
  ...
  const sorted = [...filtered].sort((a, b) => {
    const aStatus = a.stockActual === 0 ? 0 : a.stockActual <= a.stockMinimo ? 1 : 2;
    const bStatus = b.stockActual === 0 ? 0 : b.stockActual <= b.stockMinimo ? 1 : 2;
  ...
  function handleAction() {
    ...
    setInsumos((prev) =>
      prev.map((i) => {
        if (i.id !== selectedInsumo.id) return i;
        const newStock =
          dialogMode === "instalar"
            ? Math.max(0, i.stockActual - cantidad)
            : i.stockActual + cantidad;
        return { ...i, stockActual: newStock };
      })
    );
    setDialogOpen(false);
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Gestión de tóners, tintas y drums
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 stagger-children">
        ...
        <Card className={`border-border/50 bg-gradient-to-br ${insumosAlerta > 0 ? "from-amber-500/10 to-orange-600/10" : "from-emerald-500/10 to-green-600/10"}`}>
          <CardContent className="p-4">
            ...
            <p className={`text-2xl font-bold mt-1 ${insumosAlerta > 0 ? "text-amber-400" : "text-emerald-400"}`}>
              {insumosAlerta}
            </p>
          </CardContent>
        </Card>
        <Card className={`border-border/50 bg-gradient-to-br ${insumosCriticos > 0 ? "from-red-500/10 to-orange-600/10" : "from-emerald-500/10 to-green-600/10"}`}>
          <CardContent className="p-4">
            ...
            <p className={`text-2xl font-bold mt-1 ${insumosCriticos > 0 ? "text-red-400" : "text-emerald-400"}`}>
              {insumosCriticos}
            </p>
          </CardContent>
        </Card>
            ...
            <TableBody>
              {sorted.map((insumo) => {
                ...
                const barPercent = Math.min(
                  100,
                  (insumo.stockActual / (insumo.stockMinimo * 3)) * 100
                );

                return (
                  <TableRow key={insumo.id} className="border-border/30 hover:bg-muted/20 transition-colors">
                    <TableCell>
                      <p className="text-sm font-semibold font-mono">{insumo.modelo}</p>
                    </TableCell>
                    <TableCell>
                      <span className="text-sm">{tipoLabel(insumo.tipo)}</span>
                    </TableCell>
                    <TableCell>
                      <span className="text-sm text-muted-foreground">{insumo.marca || "—"}</span>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-baseline gap-1.5">
                        <span className={`text-lg font-bold tabular-nums ${
                          insumo.stockActual === 0 ? "text-red-400" : insumo.stockActual <= insumo.stockMinimo ? "text-amber-400" : "text-foreground"
                        }`}>
                          {insumo.stockActual}
                        </span>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1.5">
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-7 text-xs border-border/50 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/30"
                          disabled={insumo.stockActual === 0}
                          onClick={() => openDialog(insumo, "instalar")}
                        >
                          − Instalar
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-7 text-xs border-border/50 hover:bg-emerald-500/10 hover:text-emerald-400 hover:border-emerald-500/30"
                          onClick={() => openDialog(insumo, "recibir")}
                        >
                          + Recibir
                        </Button>
                      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {dialogMode === "instalar" ? "Instalar Insumo" : "Recibir Insumo"}
            </DialogTitle>
            <DialogDescription>
              {dialogMode === "instalar"
                ? `Restar stock de ${selectedInsumo?.modelo}`
                : `Agregar stock a ${selectedInsumo?.modelo}`}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                ...
                <Input
                  type="number"
                  min={1}
                  max={dialogMode === "instalar" ? selectedInsumo?.stockActual || 1 : 100}
                  value={cantidad}
            ...
            <div className="p-3 rounded-lg bg-muted/30 border border-border/30">
              <p className="text-sm">
                ...
                <span className="font-bold text-primary">
                  {selectedInsumo
                    ? dialogMode === "instalar"
                      ? Math.max(0, selectedInsumo.stockActual - cantidad)
                      : selectedInsumo.stockActual + cantidad
                    : 0}
                </span>
              </p>
            </div>
